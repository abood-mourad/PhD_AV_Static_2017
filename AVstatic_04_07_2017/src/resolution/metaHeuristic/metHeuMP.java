...
import resolution.Main.AV_Static;
	...
	public void Solve(PrintStream printStream, long t)
	{
		try
		{
			...
			if(solved == Boolean.TRUE) 										//if the master problem is already solved.
			{
				...
				printStream.println("Instance \t Zones \t MPs \t Offers \t Requests \t Exec_Time(s) \t Participants \t Rate(%) \t Request Satisfied \t Savings(km)");
				printStream.println(data.instance_id + "\t\t 6 \t" + data.meeting_points.size() + "\t\t" + AV_Static.num_offers + "\t\t" + AV_Static.num_requests + "\t\t" + df.format((AV_Static.getCpuTime()-t)*0.000000001) + "\t\t\t" + Obj_Val + "\t\t\t" + df.format((Obj_Val/(AV_Static.num_offers + AV_Static.num_requests))*100) + "\t\t" + df.format((Obj_Val/AV_Static.num_requests)*100)+ "\t\t" + df.format(global_saving));
				//printStream.println(data.instance_id + "\t\t 6 \t" + data.meeting_points.size() + "\t\t" + AV_Static.num_offers + "\t\t" + AV_Static.num_requests + "\t\t" + df.format((System.currentTimeMillis()-t)*0.001) + "\t\t\t" + Obj_Val + "\t\t\t" + df.format((Obj_Val/(AV_Static.num_offers + AV_Static.num_requests))*100) + "\t\t" + df.format((Obj_Val/AV_Static.num_requests)*100)+ "\t\t" + df.format(global_saving));
				printStream.println("==============================================================================================================");
			}
			else 
			{
